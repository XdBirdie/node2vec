...
sealed trait Vector extends Serializable {
   ...
   */
  def toSparseWithSize(nnz: Int): SparseVector
}
...
object Vectors {
  ...
  def norm(vector: Vector, p: Double): Double = {
    require(p >= 1.0, "To compute the p-norm of the vector, we require that you specify a p>=1. " +
      s"You specified p=$p.")
    val values: Array[Double] = vector match {
    ...
    if (p == 1) {
      ...
      var i = 0
      while (i < size) {
        val value: Double = math.abs(values(i))
  ...
  def sqdist(v1: Vector, v2: Vector): Double = {
    ...
    (v1, v2) match {
      case (v1: SparseVector, v2: SparseVector) =>
        val v1Values: Array[Double] = v1.values
        val v1Indices: Array[Int] = v1.indices
        val v2Values: Array[Double] = v2.values
        val v2Indices: Array[Int] = v2.indices
        val nnzv1: Int = v1Indices.length
        val nnzv2: Int = v2Indices.length
      ...
      case (DenseVector(vv1), DenseVector(vv2)) =>
        var kv = 0
        val sz: Int = vv1.length
        while (kv < sz) {
          val score: Double = vv1(kv) - vv2(kv)
          squaredDistance += score * score
          kv += 1
        }
      case _ =>
  ...
  def sqdist(v1: SparseVector, v2: DenseVector): Double = {
    var kv1 = 0
    var kv2 = 0
    val indices: Array[Int] = v1.indices
    var squaredDistance = 0.0
    val nnzv1: Int = indices.length
    val nnzv2: Int = v2.size
    var iv1: Int = if (nnzv1 > 0) indices(kv1) else -1
  ...
  def equals(
    ...
    val v1Size: Int = v1Values.length
    val v2Size: Int = v2Values.length
    var k1 = 0
    var k2 = 0
    var allEqual = true
    while (allEqual) {
...
class DenseVector(val values: Array[Double]) extends Vector {
  ...
  override def hashCode(): Int = {
    var result: Int = 31 + size
    var i = 0
    val end: Int = values.length
    var nnz = 0
    while (i < end && nnz < Vectors.MAX_HASH_NNZ) {
      val v: Double = values(i)
      if (v != 0.0) {
        result = 31 * result + i
        val bits: Long = java.lang.Double.doubleToLongBits(values(i))
        result = 31 * result + (bits ^ (bits >>> 32)).toInt
        nnz += 1
      }
      i += 1
  ...
  override def toSparseWithSize(nnz: Int): SparseVector = {
...
class SparseVector(
  ...
  override def asBreeze: BV[Double] = new BSV[Double](indices, values, size)
  ...
  override def hashCode(): Int = {
    ...
    while (k < end && nnz < Vectors.MAX_HASH_NNZ) {
      val v: Double = values(k)
      if (v != 0.0) {
        val i: Int = indices(k)
        result = 31 * result + i
        val bits: Long = java.lang.Double.doubleToLongBits(v)
        result = 31 * result + (bits ^ (bits >>> 32)).toInt
        nnz += 1
      }
      k += 1
  ...
  override def argmax: Int = {
    if (size == 0) {
      ...
      var maxIdx: Int = indices(0)
      var maxValue: Double = values(0)
      var maxJ = 0
      var j = 1
      val na: Int = numActives
      while (j < na) {
        val v: Double = values(j)
        if (v > maxValue) {
          maxValue = v
          maxIdx = indices(j)
          maxJ = j
  ...
  def slice(selectedIndices: Array[Int]): SparseVector = {
    ...
    val (sliceInds, sliceVals) = selectedIndices.flatMap { origIdx =>
      val iIdx: Int = java.util.Arrays.binarySearch(this.indices, origIdx)
      val i_v: Iterator[(Int, Double)] = if (iIdx >= 0) {
