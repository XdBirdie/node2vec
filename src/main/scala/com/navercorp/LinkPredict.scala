...
//      val negative: List[Long] = nodeList.diff(neighbors).toList
//      val samples: Array[Long] = Random.shuffle(negative).take(n).toArray
//      logger.warn(s"cnt: $cnt, n: $n, negative size: ${negative.length}, samples size: ${samples.length}")
      cnt += 1
      if (cnt % 100000 == 0) {
  ...
  def edgeEmbedding(edgeList: Array[(Long, Long)], embedding: Array[(Long, Array[Double])]): Array[Array[Double]] = {
    ...
    edgeList.map { case (u, v) =>
      if (map.contains(u) && map.contains(v)) {
        ...
        vec1.zip(vec2).map { case (x1, x2) => x1 * x2 }
      } else null
    }.filter((_: Array[Double]) != null) //.map((_: Array[Double]).sum)
  ...
  def setup(context: SparkContext, param: Main.Params): this.type = {
    this.sc = context
    this.numPartition = param.numPartitions
...
//val sum: Int = dataset.sortBy(_._1).zipWithIndex.map { case ((_, label), rnk) =>
//  if (label == 1) rnk + 1 else 0
//}.sum
...
//    val n1: Array[Long] = embedding.map { case (u, _) => u }.sorted.distinct
//    val n2: Array[Long] = nodeList.sorted
//    logger.warn(s"n1: ${n1.length}, n2: ${n2.length}")


//    val negativeEdge: Array[(Long, Long)] = negSampleByPair(edgeList, nodeList, 1)
    ...
    val dataset: Array[(Double, Int)] =
      edgeEmbedding(edgeList, embedding).map((x: Array[Double]) => (x.sum, 1)) ++
      edgeEmbedding(negativeEdge, embedding).map((x: Array[Double]) => (x.sum, 0))
    logger.warn(s"dataset get! size: ${dataset.length}")

    logger.warn(s"Area under ROC = ${calAUC(dataset)}")
    logger.warn(s"Area under ROC 2 = ${calAUC2(dataset)}")


//    val datasetRDD: RDD[LabeledPoint] = sc.makeRDD(dataset, numPartition).map {
//      case (features, label) =>  LabeledPoint(label, Vectors.dense(features))
//    }.cache()
//
//    val splits: Array[RDD[LabeledPoint]] = datasetRDD.randomSplit(Array(0.7, 0.3), seed = 1029L)
//    val trainSet: RDD[LabeledPoint] = splits(0).cache()
//    val testSet: RDD[LabeledPoint] = splits(1).cache()
//
//    val svmModel: SVMModel = SVMWithSGD.train(trainSet, 10)
//    val scoreWithLabels: RDD[(Double, Double)] = testSet.map { point: LabeledPoint => {
//      val score: Double = svmModel.predict(point.features)
//      (score, point.label)
//    }}
//
//    val metrics = new BinaryClassificationMetrics(scoreWithLabels)
